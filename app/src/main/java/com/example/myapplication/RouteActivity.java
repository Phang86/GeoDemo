...
import static com.example.myapplication.utils.MapUtil.convertToLatLng;
import static com.example.myapplication.utils.MapUtil.convertToLatLonPoint;
...
import com.example.myapplication.overlay.BusRouteOverlay;
import com.example.myapplication.overlay.DrivingRouteOverlay;
import com.example.myapplication.overlay.RideRouteOverlay;
...
public class RouteActivity extends AppCompatActivity implements AMapLocationListener,LocationSource,
    ...
    //出行方式数组
    private static final String[] travelModeArray = {"步行出行", "騎行出行","駕車出行","公交出行"};
    //出行方式值
...
//                etStartAddress.setEnabled(false);//禁用输入
        ...
        if (code == AMapException.CODE_AMAP_SUCCESS) {
            if (busRouteResult != null && busRouteResult.getPaths() != null) {
                if (busRouteResult.getPaths().size() > 0) {
                    ...
                    MainActivity.showMsg(RouteActivity.this,"对不起，没有搜索到相关数据！");
                }
            } else {
                MainActivity.showMsg(RouteActivity.this,"对不起，没有搜索到相关数据！");
            }
        } else {
            MainActivity.showMsg(RouteActivity.this,"错误码；" + code);
        }
    }
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_UP) {
            ...
            if (startAddress.isEmpty()) {
                MainActivity.showMsg(RouteActivity.this,"请输入当前的出发地");
                return false;
            }
            //判断目的地是否有值
            if (endAddress.isEmpty()) {
                MainActivity.showMsg(RouteActivity.this,"请输入要前往的目的地");
                return false;
            }
    ...
    public void onGeocodeSearched(GeocodeResult geocodeResult, int rCode) {
        if (rCode == PARSE_SUCCESS_CODE) {
            ...
            MainActivity.showMsg(RouteActivity.this,"获取坐标失败");
        }
    }
